...
void drive_hvd190d(hvd190d_pi::wf& p_wf, std::atomic<bool>& ref_is_running, std::atomic<bool>& ref_is_on_thread_driv)
{
...
int main(int args_len, char * args[]) 
{
    ...
    wf_main.set_param_wf(0, 16384, 100000, 1, 10, 60, 60, 0);
    wf_main.set_param_wf(1, 400*2, 100000, 3, 400, 30, 30, 0);
    wf_main.run_wf_differential();
    std::atomic<bool> is_running { false }; 
    std::atomic<bool> is_on_thread_driv { true }; 
    std::thread t_driv(drive_hvd190d, std::ref(wf_main), std::ref(is_running), std::ref(is_on_thread_driv));
